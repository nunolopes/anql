...
%
%		* domain(-Dom)
%		Return the domain annotation the loaded RDF graph is specific to
...
domain_option(State, Options) :-
	(   option(domain(Dom), Options)
	->  ttl_state_domain(State, Dom)
	;   true
	).
...
triples(State, []) -->
	[ '@', name(domain) ], !,
	iri(State, URI),
	{ set_domain_of_ttl_state(URI, State) }.
triples(State, []) -->
	[ '@', name(prefix), ':' ], !,
	iri(State, URI),
...
object_list(State, Subject, Predicate,
	    [rdf(Subject, Predicate, Object, Annotation)|T0], T) -->
