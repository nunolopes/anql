...
:- use_module('ardf/ardf_entailment').
...
sparql_compile(Query, sparql_query(Parsed, ReplyTemplate, Module), Options) :-
        ...
        Module = ardf_entailment,  % we take care of entailment regime
...
% assert the list?
read_file(File, Q, Dataset, Query) :-
        open(File, read, Stream),
        read(Stream, QueryList),
        close(Stream),
        memberchk(query(Q, Dataset, Query, _Res), QueryList).

get_result(File, Q, Res) :-
        open(File, read, Stream),
        read(Stream, QueryList),
        close(Stream),
        memberchk(query(Q, _Dataset, _Query, Res), QueryList).


% -------------------------------------------
t(X):-
        rdf_db:rdf_load('sparql/foaf.rdf'),
        sparql_query("PREFIX foaf: <http://xmlns.com/foaf/0.1/> select * from <http://nunolopes.org/foaf.rdf> where { ?s a foaf:Person }", X, []).
