...
:- multifile(built_in_function/4).
:- multifile(built_in_function/2).
:- multifile(built_in_function/1).
...
merge_assoc(A1, A2, A3) :- 
        assoc_to_list(A1, A1List),
        assoc_to_list(A2, A2List),
        union(A1List, A2List, Res),
        list_to_assoc(Res, A3).


instanciate_optional([], _, true).
instanciate_optional([Name=Var|Vars], New, (Var '=' Var2 ',' Inst)) :-
        memberchk(Name=Var2, New),
        instanciate_optional(Vars, New, Inst).
...
resolve_query((A0,B0), Q, S0, S) :- !,
	...
	mkconj(A, B, Q).
resolve_query((A0;B0), (A;B), S0, S2) :- !,
% 	resolve_query(A0, A, S0, S1),
% 	resolve_query(B0, B, S1, S).
        state_var_list(S0, VarList0),
        state_var_assoc(S0, VarAssoc0),
	resolve_query(A0, A, S0, S1),
        state_var_list(S1, VarList1),
        state_var_assoc(S1, VarAssoc1),
        set_var_list_of_state(VarList0, S1, S1a),
        set_var_assoc_of_state(VarAssoc0, S1a, S1b),
	resolve_query(B0, B, S1b, S2a),
        state_var_list(S2a, VarList2),
        state_var_assoc(S2a, VarAssoc2),
        union(VarList0, VarList1, VarList1a),
        union(VarList1a, VarList2, VarList),
        set_var_list_of_state(VarList, S2a, S2b),
        merge_assoc(VarAssoc0, VarAssoc1, VarAssoc1a),
        merge_assoc(VarAssoc1a, VarAssoc2, VarAssoc),
        set_var_assoc_of_state(VarAssoc, S2b, S2).
resolve_query(optional(true), true, S, S) :- !.
resolve_query(optional(Q0), (Q ; Inst), S0, S) :- !,
        state_var_list(S0, VarList),
	resolve_query(Q0, Q, S0, S),
        state_var_list(S, VarList2),
        instanciate_optional(VarList, VarList2, Inst).
% resolve_query(optional(Q0), (Q *-> true ; Inst), S0, S) :- !,
%         state_var_list(S0, VarList),
% 	resolve_query(Q0, Q, S0, S),
%         state_var_list(S, VarList2),
%         instanciate_optional(VarList, VarList2, Inst).
resolve_query(rdf(Subj0,P0,O0, A0), RDF, S0, S) :- !,
	resolve_graph_term(Subj0, Subj, S0, S1),
	resolve_graph_term(P0, P, S1, S2),
	resolve_graph_term(O0, O, S2, S3),
	resolve_graph_term_annot(A0, A1, A2, S3, S),
	rdf_goal(Subj, P, O, A1, A2, RDF, S).
...
rdf_goal(S, P, O0, A1, A2, RDF, State) :-
	rdf_goal_object(O0, O),
	(   state_graph(State, [Graph|_])
	->  RDF = rdf(S, P, O, A1, A2, Graph:_)
	;   RDF = rdf(S, P, O, A1, A2)
	).
...
%% HERE
resolve_graph_term_annot(Var, Var, Var, S, S) :-
	var(Var), !.
resolve_graph_term_annot(var(Name), Var1, Var2, S0, S) :- !,
	resolve_var_annot(Name, Var1, Var2, S0, S).
resolve_graph_term_annot(T, T, T, S, S).

resolve_var_annot(Name, Var1, Var2, State, State1) :-
	state_var_assoc(State, Vars),
	get_assoc(Name, Vars, Var1), !,
        assoc_to_list(Vars, VarsList),
        replaceVarAssocList(VarsList, Name, Var2, VarsList1),
        list_to_assoc(VarsList1, Vars1),
        set_var_assoc_of_state(Vars1, State, State1a),
	state_var_list(State, VL),
        replaceVarList(VL, Name, Var2, VLNew),
%        delete(VL, Name=_, VL1),
%        set_var_list_of_state([Name=Var2|VL1], State1a, State1).
        set_var_list_of_state(VLNew, State1a, State1).
resolve_var_annot(Name, Var, Var, State0, State) :- !,
...
resolve_var_annot(_, '$null$', '$null$', State, State).


replaceVarList([Name=_|List], Name, Var, [Name=Var|List]) :- !.
replaceVarList([A|Rest], Name, Var, [A|List]):-
        replaceVarList(Rest, Name, Var, List).

replaceVarAssocList([Name-_|List], Name, Var, [Name-Var|List]) :- !.
replaceVarAssocList([A|Rest], Name, Var, [A|List]):-
        replaceVarAssocList(Rest, Name, Var, List).
...
built_in_function(Function) :- 
        ardf:bb_get(domain, Dom), nonvar(Dom),
        catch(Dom:built_in_function(Function), _E, false).
...
make_triples_same_subject([property(P,(O,A))|TP], S, [rdf(S,P,O, A)|T0], T) :- !,
	make_triples_same_subject(TP, S, T0, T).
make_triples_same_subject([property(P,O)|TP], S, [rdf(S,P,O,_)|T0], T) :-
	make_triples_same_subject(TP, S, T0, T).
...
object(Obj, Triples, Tail) -->
	graph_node(Obj_, Triples_, Tail_),
        (
         annotation_or_var(A) -> {Obj = (Obj_, A), Triples = Triples_, Tail = Tail_};
         {Obj = Obj_, Triples = Triples_, Tail = Tail_}
        ).


annotation_or_var(A) --> %":", %sparql_grammar:skip_ws,
        (
         annotation(A) -> {true}; var(A)
        ),
        sparql_grammar:skip_ws.
...
primary_expression(E) --> annotation(E), !.
...
% check for domain specific functions
built_in_function(Function,           Type) :- 
        ardf:bb_get(domain, Dom), nonvar(Dom),
        catch(Dom:built_in_function(Function, Type), _E, true).
