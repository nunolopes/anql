...
eval(Var, unbound(Var)) :-
	var(Var), !.
% necessary since we have numerics as annotation values for the fuzzy case
eval(F, Result) :-
	float(F), !,
        atom_number(FA, F),
        eval_literal(type('http://www.w3.org/2001/XMLSchema#decimal',FA),Result).
% necessary since we have numerics as annotation values for the fuzzy case
eval(I, Result) :-
	integer(I), !,
        atom_number(IA, I),
        eval_literal(type('http://www.w3.org/2001/XMLSchema#integer',IA), Result).
...
% % cannot be done here due to the use of the clause predicate to find all the op's
% % check for domain specific functions
% op(Op, Result) :- 
%         ardf:bb_get(domain, Dom), nonvar(Dom),
%         catch(Dom:op(Op, Result), _E, false).
